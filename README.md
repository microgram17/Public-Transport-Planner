...
Pydantic validates environment configuration and the data returned by repositories. Application queries under
`backend/src/app/db` use explicit SQL through pooled Psycopg connections, and the GTFS importer uses Psycopg `COPY` for
bulk loading. Models under `backend/src/app/models` are Pydantic data models; the application has no SQLAlchemy ORM.

Alembic remains the migration runner and therefore still brings SQLAlchemy as its own dependency. SQLAlchemy is confined
to the migration environment and migration scripts. Because there is no application SQLAlchemy metadata, migrations are
written and reviewed explicitly instead of being autogenerated.
